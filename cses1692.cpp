...
int main() {
	...
	const uint32_t N = (1 << 15) + 1;
	uint32_t TC = 1, n, sn, rn, mask, s[N];
	uint8_t res[N], pf[N];
	//cin >> TC;
	while (TC--) {
		cin >> n;
		...
		n--;
		mask = (1 << n) - 1;
		memset(pf, 1, sizeof(pf[0]) * (1 << n));
		rn = sn = 0;
		s[++sn] = 0;
		while (sn) {
			uint32_t last = s[sn];
			if (pf[last] != UCHAR_MAX) {
				s[++sn] = ((last << 1) | pf[last]) & mask;
				--pf[last];
			} else {
				--sn;
				res[++rn] = ((s[sn] << 1) & mask) != last;
			}
		}
		for (uint32_t i = 1; i < n; ++i) cout << '0';
