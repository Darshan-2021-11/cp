...
void radix_sort(uint32_t *a, uint32_t *b, uint32_t n) {
	uint32_t freq[4][256], *q[256], *src, *dst;
	for (uint32_t j = 0; j < 4; ++j) {
		for (uint32_t i = 0; i < 256; ++i) freq[j][i] = 0;
		for (uint32_t i = 0; i < n; ++i) ++freq[j][(a[i] >> (j << 3)) & 0xff];
	}
	for (uint32_t j = 0; j < 4; ++j) {
		src = j & 1 ? b : a;
		dst = j & 1 ? a : b;
...
int main() {
	...
	const int N = 100'000;
	uint32_t TC = 1, a[N], b[N], v[2], n, q, l, r, x;
	//cin >> TC;
	while (TC--) {
		cin >> n;
		for (uint32_t i = 0; i < n; ++i) cin >> a[i];
		for (uint32_t i = 0; i < n; ++i) a[i] ^= 1U << 31;
		radix_sort(a, b, n);
		cin >> q;
		while (q--) {
			cin >> v[0];
