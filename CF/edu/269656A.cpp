...
static const i32 N = 400'000;
static i32 n;
static i64 sub;
template <i32 N>
struct suffix_array {
  i32 p[N + 1] {}, lcp[N + 1] {}, _n; char data[N + 1];
  suffix_array() : _n(0) { while ((data[_n] = getchar()) != '\n') ++_n; make(); }
  suffix_array(const char *source, i32 size) : _n(size) { assert(N >= 256); copy(source, source + size, data); make(); }
  void make() { i32 cn[N + 1], pn[N + 1], c[N + 1]; array<i32, 2> pre, cur; data[_n++] = '\0';
    {
      fill_n(lcp, 256, 0); for (i32 i = 0; i < _n; ++i) ++lcp[c[i] = data[i] - '\0']; for (i32 i = 0; i < 256; ++i) pn[i] = lcp[i] != 0;
      partial_sum(lcp, lcp + 256, lcp); for (i32 i = 0; i < _n; ++i) p[--lcp[c[i]]] = i;
      pn[0] = 0; partial_sum(pn, pn + 256, pn); for (i32 i = 0; i < _n; ++i) c[i] = pn[c[i]];
    }
...
int main() {
  ...
  suffix_array<N> s;
  sub = (static_cast<i64>(s._n) * (s._n - 1)) >> 1;
  for (i32 i = 0; i < s._n - 1; ++i) sub -= s.lcp[i];
  printf("%lld", sub);
}
