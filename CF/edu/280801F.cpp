...
struct DynamicLazySegTree {
	...
	DynamicLazySegTree(const uint32_t size) noexcept {
		init(size);
	}
	int get() noexcept {
		assert(_n < N);
...
int main() {
	...
	const int N = 6'000'000; uint32_t TC = 1, n, a, b; int d, h; char c;
	while (TC--) {
		cin >> n;
		DynamicLazySegTree<int, N> dst(n);
		while (true) {
			cin >> c;
			if (c == 'E') break;
			if (c == 'I') {
