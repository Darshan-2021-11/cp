...
int main() {
  ...
  while (T--) {
    n = 0; while ((a[n] = getchar()) != '\n') ++n;
    io = ie = 0;
    while (true) {
      while (io < n && !(a[io] & 1)) ++io;
      while (ie < n &&  (a[ie] & 1)) ++ie;
      if (ie == n | io == n) break;
      if (a[io] < a[ie]) putchar(a[io++]);
      else               putchar(a[ie++]);
    }
    for (; io < n; io++) if (  a[io] & 1 ) putchar(a[io]);
    for (; ie < n; ie++) if (!(a[ie] & 1)) putchar(a[ie]);
    putchar('\n');
  }
}
