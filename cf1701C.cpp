...
int main() {
	...
	while (TC--) {
		...
		for (int i = 0; i < n; ++i) if (a[i]) pq.push(make_pair(a[i], i)), um.push_back(i); else un.push_back(i);
		for (int j = 0; j <= 4e5 + 1; ++j) {
			while (dq.size()) {
				auto [x, y] = dq.front();
				if (x != j) break;
				else {
					if (!a[y]) un.push_back(y);
					else um.push_back(y);
				}
				dq.pop_front();
			}
...
rep:
				...
				pq.pop();
				if (a[y] != x) {
					if (a[y]) pq.push(make_pair(a[y], y));
					goto rep;
				}
				if (a[y]) {
					pq.push(make_pair(--a[y], y));
					dq.push_back(make_pair(j + 2, z));
				} else goto rep;
			}
			if (dq.size() == 0) {
				cout << j << '\n';
