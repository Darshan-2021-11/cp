...
int main() {
	...
	while (TC--) {
		...
		while (l < r) {
			...
			vector<int> idx;
			for (int i = 0; i < n; ++i) {
				if (c[i] > x) idx.push_back(a[i]);
			}
			if (is_sorted(idx.begin(), idx.end())) {
				r = x;
				v = min<int>(v, idx.size());
			} else {
				l = x + 1;
			}
		}
		...
		s.insert({ n, INT_MAX });
		for (int i = 0; i < n; ++i) {
			if (c[i] > l) s.insert({ i, a[i] });
		}
		for (int i = 0; i < n; ++i) {
			if (c[i] <= l) {
				auto it = s.upper_bound({ i, 0 });
				auto it2 = --s.upper_bound({ i, 0 });
				if (it->second >= a[i] && it2->second <= a[i]) {
					s.insert({ i, a[i] });
				}
			}
		}
