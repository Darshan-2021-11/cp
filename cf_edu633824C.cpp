...
int main() {
	...
	const int N = 1'001;
	int TC = 1, n, m, m1, b[N << 1], d[N << 1], c[N];
	array<int, 2> a[N];
	bool pos;
	//cin >> TC;
	while (TC--) {
		...
		m1 = 0;
		cin >> n;
		for (int i = 0; i < n; ++i) cin >> a[i][0] >> a[i][1], c[i] = 0;
		sort(a, a + n);
		cin >> m;
		...
		int pre = 0, cur = 0;
		for (int i = 0; i < n; ++i) {
			while (b[pre] <  a[i][0]) ++pre;
			while (b[cur] <= a[i][1]) ++cur;
			if (cur - pre > 2) {
				pos = false;
				break;
			}
		...
		*/
		...
		for (int i = 0; i < n - 1; ++i) {
			...
			while (b[cur] <  a[i][1]) ++cur;
			if (c[i] == 2) continue;
			if (a[i][1] == a[i + 1][0] && b[cur] != a[i][1] && c[i + 1] < 2) {
				d[m1++] = a[i + 1][0], ++c[i], ++c[i + 1];
		...
		if (c[n - 1] != 2) {
			while (b[pre] <= a[n - 1][0]) ++pre;
			for (int j = a[n - 1][0] + 1; j < a[n - 1][1]; ++j) {
				if (j == b[pre]) continue;
				++c[n - 1];
				d[m1++] = j;
				if (c[n - 1] == 2) break;
