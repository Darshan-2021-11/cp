...
int main() {
	...
	const int N = 200'001;
	int64_t dp[N], pc, tmp;
	int TC = 1, n, m, ef[N << 2], pf[N];
	cin >> n >> m;
	dp[0] = 1, pc = 1;
	memset(dp + 1, 0, sizeof(dp[0]) * m);
	memset(pf + 1, 0, sizeof(pf[0]) * m);
	for (int t = 1, u; t <= n << 2;) {
		cin >> u >> ef[t++];
		ef[t++] = pf[u];
		cin >> ef[t++];
		cin >> ef[t++];
		(pc *= pbyq(ef[t - 1] - ef[t - 2], ef[t - 1])) %= MOD;
		if (pc >= MOD)
			pc -= MOD;
		pf[u] = t - 4;
	}
	for (int i = 1; i <= m; ++i) {
		for (int j = pf[i]; j; j = ef[j + 1]) {
			tmp = pbyq(dp[i - 1] * ef[j + 3] % MOD, ef[j + 3] - ef[j + 2]);
			(tmp *= pbyq(ef[j + 2], ef[j + 3])) %= MOD;
			dp[ef[j]] += tmp;
			if (dp[ef[j]] >= MOD)
				dp[ef[j]] -= MOD;
		}
