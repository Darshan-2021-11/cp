...
int main() {
	...
	const uint32_t N = ((1u << 20) + 1) * 20, MOD = 1e9 + 7;
	uint32_t TC = 1u, n, m, ef[401u << 1u], pf[21u], dp[N];
	//cin >> TC;
	while (TC--) {
		cin >> n >> m;
		memset(pf, UINT8_MAX, sizeof(pf[0]) * n);
		memset(dp + 1u, 0, sizeof(dp[0]) * (1u << n) * n);
		for (uint32_t i = 0, t = 1u, u; i < m; ++i) {
			cin >> u;
			--u;
			cin >> ef[t];
			--ef[t++];
		...
		dp[n] = 1u;
		for (uint32_t i = 1u; i < ((1u << n) - 1u); ++i) {
			for (uint32_t j = 0; j < n; ++j) {
				if (!dp[i * n + j]) continue;
				for (uint32_t k = pf[j]; k != UINT32_MAX; k = ef[k + 1u]) {
					if ((ef[k] == n - 1u && (i | (1u << ef[k])) != ((1u << n) - 1u)) || (i | (1u << ef[k])) == i) continue;
					dp[(i | (1u << ef[k])) * n + ef[k]] += dp[i * n + j];
					if (dp[(i | (1u << ef[k])) * n + ef[k]] >= MOD) dp[(i | (1u << ef[k])) * n + ef[k]] -= MOD;
				}
			}
		}
		/*
