...
int main() {
	...
	while (TC--) {
		...
		while (l < r) {
			...
			pts = 0;
			if ((a[0] - x + 1) > 0) pts += a[0] - x + 1;
			if (mx - (a[n - 1] + x) + 1 > 0) pts += mx - (a[n - 1] + x) + 1;
			for (int i = 1; i < n; ++i) {
				if ((a[i] - x) - (a[i - 1] + x) + 1 > 0) pts += (a[i] - x) - (a[i - 1] + x) + 1;
			}
eval:
			if (pts < k) r = x - 1;
			else l = x;
