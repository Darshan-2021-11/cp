...
int main() {
	...
	const uint32_t N = 100'001, M = 200'001, INF = UINT32_MAX;
	uint64_t cst[N], cost[N], mx;
	uint32_t TC = 1, ef[M << 1], s[M], pf[N][3], tpf[N], cs[N], *id = cs, *q = s, n, m, sp, csp, timer, cid, qf, qb, c, u, v;
	while (TC--) {
		cst[0] = 0;
		timer = cid = 0;
		cin >> n >> m;
		memset(pf + 1, 0, sizeof(pf[0][0]) * 3 * n);
		for (uint32_t i = 1; i <= n; ++i) cin >> cst[i];
		for (uint32_t i = 0, t = 1; i < m; ++i) {
			cin >> u >> v;
			ef[t++] = v, ef[t++] = pf[u][2], tpf[u] = pf[u][2] = t - 2;
		}
		// SCC(component ID stored in pf[i][2] with the id of smallest node)
		for (uint32_t t, w, i = 1; i <= n; ++i) {
			if (pf[i][0]) continue;
			pf[i][0] = pf[i][1] = ++timer;
			s[++(sp = 0)] = i, cs[++(csp = 0)] = i;
			while (sp) {
				if (pf[(u = s[sp])][2]) {
					t = pf[u][2], pf[u][2] = ef[t + 1], v = ef[t];
					if (pf[v][0]) {
						if (pf[u][1] > pf[v][0]) { // not bridge edge
							pf[u][1] = pf[v][0];
						}
					}
					else {
						pf[v][0] = pf[v][1] = ++timer, s[++sp] = cs[++csp] = v;
					}
				} else {
					if (pf[u][0] == pf[u][1]) {
						//++cid;
						mx = 0;
						do w = cs[csp--], pf[w][0] = INF, pf[w][2] = /* cid */u, mx += cst[w]; while (w != u);
						cst[u] = mx;
					}
					t = s[--sp];
					if (pf[t][1] > pf[u][1]) { // not bridge edge
						pf[t][1] = pf[u][1];
					}
				}
			}
		}
		for (uint32_t i, j = 1; j <= n; ++j) {
			for (i = tpf[j]; i;) {
				ef[i] = pf[ef[i]][2], i = ef[i + 1];
			}
		}
		for (uint32_t i = 1, j; i <= n; ++i) {
			if (pf[i][2] == i) continue;
			while (tpf[i]) {
				j = tpf[i];
				tpf[i] = ef[j + 1];
				ef[j + 1] = tpf[pf[i][2]];
		...
		timer = 0;
		memset(cs + 1, 0, sizeof(cs[0]) * n);
		for (uint32_t i = 1; i <= n; ++i) {
			if (pf[i][2] != i) continue;
			timer++;
			uint32_t prev = 0;
			uint32_t current = tpf[i];
			while (current != 0) {
				v = ef[current];
				if (v == i || cs[v] == timer) {
					if (prev == 0) {
						...
						ef[prev + 1] = ef[current + 1];
					}
					current = ef[current + 1];
				} else {
					cs[v] = timer;
					prev = current;
					current = ef[current + 1];
		...
		/*
		{
			memset(id + 1, 0, sizeof(id[0]) * n);
			for (uint32_t i = 1; i <= n; ++i) {
				for (uint32_t j = tpf[i]; j; j = ef[j + 1]) {
					v = ef[j];
					id[v]++;
				}
			}
		...
		*/
		...
		memset(cost + 1, 0, sizeof(cost[0]) * n);
		qf = qb = 0;
		for (uint32_t i = 1; i <= n; ++i) if (id[i]) continue; else q[qb++] = i, cost[i] = cst[i];
		while (qf < qb) {
			c = q[qf];
			for (uint32_t i = tpf[c]; i;) {
				u = ef[i], i = ef[i + 1];
				if (cst[u] + cost[c] > cost[u])
					cost[u] = cst[u] + cost[c];
				if (--id[u] == 0) q[qb++] = u;
			}
			++qf;
		...
		mx = 0;
		for (uint32_t i = 1; i <= n; ++i) mx = max(mx, cost[i]);
		cout << mx;
	}
}
