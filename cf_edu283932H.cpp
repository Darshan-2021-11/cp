...
int main() {
	...
	const int N = 101;
	int64_t TC = 1, cnt[3], val[3], pri[3], l, r, x, y, v;
	char c[N], *p;
	//cin >> TC;
	while (TC--) {
		memset(cnt, 0, sizeof(cnt[0]) * 3);
		cin >> c;
		p = c;
		do {
			switch(*p++) {
		...
		for (uint32_t i = 0; i < 3; ++i) cin >> val[i];
		for (uint32_t i = 0; i < 3; ++i) cin >> pri[i];
		cin >> v;
		l = 0, r = 1e12 + 100;
		while (l < r) {
			x = (l + r + 1) >> 1;
			...
			for (uint32_t i = 0; i < 3; ++i) {
				y += max<int64_t>(0, (x * cnt[i] - val[i])) * pri[i];
			}
			if (y <= v) l = x;
			else r = x - 1;
		}
