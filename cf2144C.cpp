...
void solve() {
    ...
    const int C_MAX = 200001;
    vector<int> counts(C_MAX + 1, 0);
    int max_c = 0;
    for (int i = 0; i < n; ++i) {
        int c;
        cin >> c;
    ...
    vector<long long> prefix_counts(C_MAX + 2, 0);
    for (int i = 0; i <= C_MAX; ++i) {
        prefix_counts[i + 1] = prefix_counts[i] + counts[i];
    }

    auto get_count_in_range = [&](int l, int r) {
        if (l > r) return 0LL;
        l = max(l, 0);
        r = min(r, C_MAX);
        return prefix_counts[r + 1] - prefix_counts[l];
    };

    long long max_income = -4e18; // A very small number

    for (int x = 2; x <= max_c + 1; ++x) {
        long long current_total_price = 0;
        long long printed_tags_needed = 0;

        for (long long start_v = 0; start_v <= max_c; start_v += x) {
            int p = start_v / x + 1;
            long long num_new_p = get_count_in_range(start_v + 1, start_v + x);

            if (num_new_p > 0) {
                current_total_price += p * num_new_p;
                long long old_count_p = (p <= C_MAX) ? counts[p] : 0;
                if (num_new_p > old_count_p) {
                    printed_tags_needed += (num_new_p - old_count_p);
                }
            }
        }

        long long current_income = current_total_price - y * printed_tags_needed;
