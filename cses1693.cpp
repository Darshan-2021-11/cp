...
int main() {
	...
	const int N = 100'001, M = N << 1, VISITED = INT_MAX;
	// visited described as adj. node INT_MAX
	// id is first used as indegree, then as euler path
	int TC = 1, n, m, rn, sn, ef[M << 1], id[M + 1], s[M + 1], od[N], pf[N], *res = id;
	bool pos;
	//cin >> TC;
	while (TC--) {
		pos = true;
		cin >> n >> m;
		memset(pf + 1, 0, sizeof(pf[0]) * n);
		memset(id + 1, 0, sizeof(id[0]) * n);
		memset(od + 1, 0, sizeof(od[0]) * n);
		...
		pos &= rn == m + 1;
		pos &= res[rn] == 1 && res[1] == n;
out:
		if (!pos) {
			cout << "IMPOSSIBLE\n";
		} else {
			while (rn) cout << res[rn--] << ' ';
			cout << '\n';
		}
	}
}
