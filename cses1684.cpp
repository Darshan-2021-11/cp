...
int main() {
	...
	const int N = 200'001, INF = UINT32_MAX;
	uint32_t TC = 1, ef[N << 1], pf[N][3], s[N], cs[N], n, m, sp, csp, timer, cid;
	char ans[N], c[2];
	while (TC--) {
		timer = cid = 0;
		cin >> n >> m;
		...
		for (uint32_t i = 0, t = 1, u[2], v; i < n; ++i) {
			...
			v = u[1] > m ? u[1] - m : u[1] + m;
			ef[t++] = u[0], ef[t++] = pf[v][2], pf[v][2] = t - 2;
			v = u[0] > m ? u[0] - m : u[0] + m;
			ef[t++] = u[1], ef[t++] = pf[v][2], pf[v][2] = t - 2;
		}
		// SCC
		for (uint32_t t, u, v, w, i = 1; i <= m << 1; ++i) {
			if (pf[i][0]) continue;
			pf[i][0] = pf[i][1] = ++timer;
			s[++(sp = 0)] = i, cs[++(csp = 0)] = i;
			while (sp) {
				if (pf[(u = s[sp])][2]) {
					...
					if (pf[u][0] == pf[u][1]) {
						++cid;
						do w = cs[csp--], pf[w][0] = INF, pf[w][2] = cid; while (w != u);
					}
					t = s[--sp];
