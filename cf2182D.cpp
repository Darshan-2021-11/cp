...
int main() {
	...
	while (TC--) {
		...
		while (l < r) {
			x = l + r + 1 >> 1;
			req = 0;
			for (int i = 1; i <= n; ++i) req += max<int64_t>(0, x - a[i]);
			if (req > a[0]) r = x - 1;
			else l = x;
		}
		mx = 0;
		for (int i = 1; i <= n; ++i) {
			x = max<int64_t>(0, l - a[i]);
			a[i] += x;
			a[0] -= x;
			mx = max(mx, a[i]);
		}
		if (l + 1 < mx) {
			...
			req = 0;
			for (int i = 1; i <= n; ++i) req += a[i] == mx;
			cout << (f[a[0] + req] * f[n - a[0] - req] % MOD * nCr(n - req, a[0]) % MOD) << '\n';
		} else {
			cout << (f[a[0]] * f[n - a[0]] % MOD * nCr(n, a[0]) % MOD) << '\n';
		}
	}
}
